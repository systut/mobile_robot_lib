...
tmeasure = 0.0;
state_real = [0; -0.1; pi/2];     % initial state of the system
...
% EKF parameters
n_ekf = 5;
C_d = [eye(o), zeros(o,n_ekf-o)];    % only [x; y; theta] are measured
W_d = eye(n_ekf);
V_d = eye(o);
...
Q_ekf = diag([0.1 0.1 0.2 10 10]);
% initial state
x_tilde_k = [ 0; -0.1; pi/2; 0.02; 0.02 ]; % state estimated by EKF
x_tilde_measure = [ 0; 0; 0 ];          % real [x; y; theta] values used for simulation (slip values are taken from slip_ref)
P_k_ekf = diag([0.1 0.1 0.1 10 10]);
...
for ii = 1:MPCIterations
    ...
    state_estim = x_tilde_k(1:3);
    slip_params = x_tilde_k(4:5)';
    p_estim = [p_lin, slip_params];
    ...
    for k = 0:N-1
        ...
        A_d = A_d_num_slip(state_lin, input_lin, p_estim);
        B_d = B_d_num_slip(state_lin, input_lin, p_estim);
    ...
    x_tilde_pred = f_d_num_ext(x_tilde_k, input_wmr, p_lin);
    
    A_d_ekf = A_d_num_ext(x_tilde_k, input_wmr, p_lin);
    P_pred = A_d_ekf*P_k_ekf*A_d_ekf' + W_d*Q_ekf*W_d;
    
    % system simulation
    
    % correction / measurement update eqs
    % Kalman gain
    K_ekf = P_pred*C_d' / ( C_d*P_pred*C_d' + V_d*R_ekf*V_d' ); % use / to avoid inv()
    x_tilde_k = x_tilde_pred + K_ekf*(measurement - C_d*x_tilde_pred);
    P_k_ekf = (eye(n_ekf) - K_ekf*C_d)*P_pred;
...
f6 = figure(6);
f6.Color = 'w';
plot(t_ref(1:end-N), slip_ref(1,1:end-N), '--', 'Color', grey, 'linewidth', 1.5), grid on, hold on,
plot(t_ref(1:end-N), slip_estim(1,:), '-', 'Color', blue, 'linewidth', 1.5)
...
f7 = figure(7);
f7.Color = 'w';
plot(t_ref(1:end-N), slip_ref(2,1:end-N), '--', 'Color', grey, 'linewidth', 1.5), grid on, hold on,
plot(t_ref(1:end-N), slip_estim(2,:), '-', 'Color', blue, 'linewidth', 1.5)
