...
matlabFunction(f,  'File','functions/f_num'  ,'Vars',{x,u,parameters});
matlabFunction(f_d,'File','functions/f_d_num','Vars',{x_k,u_k,parameters});
matlabFunction(A_d,'File','functions/A_d_num','Vars',{x_lin,u_lin,parameters});
matlabFunction(B_d,'File','functions/B_d_num','Vars',{x_lin,u_lin,parameters});
...
for k = 1:length(t_ref)-1
    ...
    x_sim(:,k+1) = f_d_num(x_sim(:,k),u_ref(:,k),p);
    dx    = A*dx + B*du;
